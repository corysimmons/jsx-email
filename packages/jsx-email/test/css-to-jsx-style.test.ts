...
test('transforms rules with browser prefixes', () => {
  expect(cssToJsxStyle('-webkit-transform: scale(2); -ms-transform: scale(2);')).toEqual({
    msTransform: 'scale(2)',
...
test('transforms rules with urls', () => {
  expect(cssToJsxStyle('background: red url("awesome/image.png") no-repeat; color: red')).toEqual({
...
test('transforms rules with dataurls', () => {
  expect(cssToJsxStyle('cursor: url(data:image/gif;base64,data); color: red')).toEqual({
    color: 'red',
...
test("transforms rules with quoted ';' in value", () => {
  expect(cssToJsxStyle("list-style-type: ';'; list-style-position: outside;")).toEqual({
...
test("transforms rules with quoted ':' in value", () => {
  expect(cssToJsxStyle('list-style-type: ";"; list-style-position: inside;')).toEqual({
...
test('transforms rule properties of any case to camelCase', () => {
  expect(cssToJsxStyle('FONT-SIZE: 2em; font-WEIGHT: bold; oPaCiTy: 1;')).toEqual({
    fontSize: '2em',
    fontWeight: 'bold',
    opacity: '1'
  });
...
test('handles excessive whitespace', () => {
  expect(cssToJsxStyle('color:\nred; \n\r\t\tfont-size:\r2em;\topacity:  1 ;')).toEqual({
    color: 'red',
    fontSize: '2em',
    opacity: '1'
  });
