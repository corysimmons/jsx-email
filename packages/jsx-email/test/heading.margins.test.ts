import { withMargin, withSpace, type Margin } from '../src/components/heading';
...
describe('withMargin', () => {
  ...
  it('should apply margin to specified sides when provided', () => {
    const marginProps: Margin = { mb: '15', ml: '20', mr: '10', mt: '5' };
    const result = withMargin(marginProps);
    expect(result).toEqual({
      marginBottom: '15px',
      marginLeft: '20px',
...
describe('withSpace', () => {
  ...
  it('should apply space to the specified properties', () => {
    ...
    expect(result).toEqual({ marginLeft: '15px', marginTop: '15px' });
  });
});
